...
import {
  FaTimes, FaUser, FaPhone, FaMapMarkerAlt, FaTint, FaVenusMars,
...
const MembershipCardModal = ({ onClose }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Exclude fields that shouldn't be updated loosely if needed, but here we update all editable ones
      // We don't update email usually, but if it's in formData it might overwrite. 
      // Ideally we check if email changed and warn, but let's assume valid edits.
